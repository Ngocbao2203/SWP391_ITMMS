...
const Services = () => {
  ...
  async function fetchServices() {
    ...
    try {
      ...
      safeToast(
        "error",
        "Lỗi khi tải danh sách dịch vụ: " + (error.message || "Không xác định")
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingService) {
        const response = await treatmentService.updateTreatmentService(
          editingService.id,
          formData
        );
        if (response.success) {
        ...
        const response = await treatmentService.createTreatmentService(
          formData
        );
      ...
      safeToast(
        "error",
        "Lỗi khi lưu dịch vụ: " + (error.message || "Không xác định")
      );
      console.error(error);
      setLoading(false);
    }
  ...
  const handleDeleteService = async (key) => {
    ...
    try {
      setLoading(true);
      const response = await treatmentService.deleteTreatmentService(
        service.id
      );
      if (response.success) {
      ...
      safeToast(
        "error",
        "Lỗi khi xóa dịch vụ: " + (error.message || "Không xác định")
      );
      setLoading(false);
    }
  };
        ...
        record.imageUrl && record.imageUrl.trim() ? (
          <img
            ...
            onError={(e) => {
              ...
              console.log("Image load failed for:", record.imageUrl);
            }}
          />
        ) : (
      ...
      >
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: "24px" }}>
          <Col xs={12} sm={8}>
            <Card
              size="small"
              style={{ borderRadius: "8px", background: "#e6f7ff" }}
            >
              <Statistic
                title="Tổng Dịch Vụ"
                value={totalServices}
          ...
          <Col xs={12} sm={8}>
            <Card
              size="small"
              style={{ borderRadius: "8px", background: "#fff7e6" }}
            >
              <Statistic
          ...
          <Col xs={12} sm={8}>
            <Card
              size="small"
              style={{ borderRadius: "8px", background: "#fff2e8" }}
            >
              <Statistic
                title="Tỷ Lệ TB"
            ...
            <Col span={8}>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Vui lòng nhập tỷ lệ thành công!",
                  },
                ]}
              >
                <InputNumber
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                ...
                rules={[
                  { required: false, message: "Vui lòng nhập URL hình ảnh!" },
                ]}
              >
                <Input
            ...
            <Col span={12}>
              <Form.Item
                ...
                rules={[
                  {
                    required: false,
                    message: "Vui lòng tải lên tệp hình ảnh!",
                  },
                ]}
              >
                <Upload
