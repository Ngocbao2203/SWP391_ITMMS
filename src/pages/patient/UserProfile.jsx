          ...
          setMedicalHistory([
            {
              ...
              notes:
                "Bệnh nhân cần theo dõi định kỳ, tuân thủ thuốc theo chỉ định",
    ...
    if (info.file.status === "done") {
      ...
      getBase64(info.file.originFileObj, async (imageUrl) => {
        try {
          ...
          const result = await authService.updateUserProfile(userInfo.id, {
            avatar: imageUrl,
          });
          ...
          if (result.success) {
            ...
            message.error(
              result.message || "Có lỗi xảy ra khi cập nhật ảnh đại diện"
            );
          }
        } catch (error) {
    ...
    >
      <Row gutter={[24, 24]}>
        <Col xs={24} sm={8} md={6} style={{ textAlign: "center" }}>
          ...
          <Badge
            count={
              editMode ? <CameraOutlined style={{ color: "#1890ff" }} /> : 0
            }
          >
            <Avatar
              size={120}
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={8}>
          <Statistic
            ...
            value={
              treatmentSchedule.filter((item) => item.status === "Sắp tới")
                .length
            }
        ...
        <Col xs={24} sm={8}>
          <Statistic
            title="Đã lên lịch"
            value={
              treatmentSchedule.filter((item) => item.status === "Đã lên lịch")
                .length
            }
          ...
          <List
            ...
            renderItem={(item) => (
              <List.Item
                ...
                extra={
                  item.coverImage && (
                    <img width={272} alt={item.title} src={item.coverImage} />
                  )
                }
                actions={[<Link to={`/blog/${item.id}`}>Xem chi tiết</Link>]}
              >
                <List.Item.Meta
          ...
          <div className="profile-content">
            ...
            <div className="profile-section" style={{ marginTop: 24 }}>
              <Title level={4}>
                ...
                <Badge
                  count={
                    treatmentSchedule.filter(
                      (item) => item.status === "Sắp tới"
                    ).length
                  }
                  style={{ marginLeft: 8 }}
                />
