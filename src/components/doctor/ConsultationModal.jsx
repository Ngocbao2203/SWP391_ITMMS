  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (!patientId) {
        ...
        message.error('Không thể xác định thông tin bệnh nhân. Vui lòng thử lại.');
        return;
      }
      ...
      if (!patientId) {
        ...
        message.error('Không thể xác định thông tin bệnh nhân. Dữ liệu lịch hẹn không đầy đủ.');
        return;
      }
      ...
      if (values.nextAppointmentDate) {
        try {
          nextAppointmentFormatted = values.nextAppointmentDate.format('YYYY-MM-DD');
      ...
      if (!values.symptoms || !values.diagnosis || !values.treatment) {
        ...
        message.error('Vui lòng điền đầy đủ thông tin khám bệnh (triệu chứng, chẩn đoán, điều trị)');
        return;
      }
      ...
      const medicalRecordData = {
        ...
        nextAppointmentDate: values.followUpRequired ? nextAppointmentFormatted : null
      };
      
      console.log('🔧 FIXED: Using appointment.id as appointmentId:', realAppointmentId);
      console.log('🔧 Original appointment object keys:', Object.keys(appointment));
      console.log('🔧 appointment.id:', appointment.id);
      console.log('🔧 appointment.appointmentId:', appointment.appointmentId);
      
      // VALIDATION: Check appointmentId và customerId
      console.log('✅ VALIDATION CHECKS:');
      console.log('- appointmentId type:', typeof realAppointmentId, 'value:', realAppointmentId);
      console.log('- customerId type:', typeof customerIdNumber, 'value:', customerIdNumber);
      ...
      console.log('Request body JSON:', JSON.stringify(medicalRecordData, null, 2));
      
      // 1. Create medical record
      console.log('=== CALLING adminService.completeAppointment ===');
      console.log('Method: adminService.completeAppointment');
      console.log('Params: doctorId =', doctorId, 'medicalRecordData =', medicalRecordData);
      ...
      try {
        ...
        console.log('MedicalRecords test endpoint status:', testResponse.status);
        if (testResponse.ok) {
          const testData = await testResponse.text();
          console.log('MedicalRecords test response:', testData);
        } else {
          console.warn('MedicalRecords test failed:', await testResponse.text());
        }
      } catch (testError) {
      ...
      console.log('📍 URL:', `http://localhost:5037/api/MedicalRecords/complete/${doctorId}`);
      ...
      try {
        ...
        console.log('📋 Manual API Response Headers:', Object.fromEntries(manualResponse.headers.entries()));
        ...
        if (manualResponse.ok) {
          ...
          if (createTreatmentPlan && values.treatmentServiceId) {
            try {
              ...
              const treatmentPlanData = {
                ...
                startDate: values.startDate ? values.startDate.format('YYYY-MM-DDTHH:mm:ss') : dayjs().format('YYYY-MM-DDTHH:mm:ss'),
                totalCost: 0, // Set default value, will be calculated by backend
                notes: values.notes || ''
              };

              console.log('Treatment plan payload:', JSON.stringify(treatmentPlanData, null, 2));

              const treatmentResult = await treatmentService.createTreatmentPlan(treatmentPlanData);
              ...
              if (treatmentResult && treatmentResult.success) {
                ...
                message.success('Hoàn thành khám bệnh và tạo kế hoạch điều trị thành công!');
              } else {
                console.error('=== TREATMENT PLAN CREATION FAILED ===');
                console.error('Treatment error response:', treatmentResult);
                message.warning('Medical record đã tạo thành công, nhưng có lỗi khi tạo kế hoạch điều trị: ' + (treatmentResult?.message || 'Unknown error'));
              }
              ...
              message.warning('Medical record đã tạo thành công, nhưng có lỗi khi tạo kế hoạch điều trị: ' + treatmentError.message);
            }
          } else {
          ...
          try {
            ...
            console.error('📝 Error Message:', errorData.message || errorData.title || errorData.error);
            console.error('⚠️ Validation Errors:', errorData.errors || errorData.validationErrors);
          } catch (parseError) {
            console.error('❌ Could not parse error response as JSON');
          }
          
          // Show specific error to user
          const errorMsg = errorData?.message || errorData?.title || `HTTP ${manualResponse.status}: ${errorText}`;
      ...
      message.error('Có lỗi xảy ra khi hoàn thành khám bệnh: ' + (error.message || 'Unknown error'));
    } finally {
  ...
  return (
    <Modal
      title={
        <Space>
          <MedicineBoxOutlined />
          <span>Khám bệnh - {appointment.customerName || `Bệnh nhân ${appointment.customerId}`}</span>
        </Space>
      }
      open={visible}
      onCancel={onCancel}
      ...
      {loading ? (
        ...
        >
            ...
            <Row gutter={16}>
              ...
              <Col span={20}>
                <Space direction="vertical" size={4}>
                  <Title level={4} style={{ margin: 0 }}>{patientData?.name}</Title>
                  <Text>Tuổi: {patientData?.age} | Giới tính: {patientData?.gender}</Text>
                  <Text>Điện thoại: {patientData?.phone}</Text>
                  {existingTreatmentPlan && (
                    <Tag color="blue">
                      Đang điều trị: {existingTreatmentPlan.treatmentType} - Giai đoạn {existingTreatmentPlan.currentPhase}
                    </Tag>
                  )}
                </Space>
              </Col>
          ...
          <Row gutter={16}>
            ...
            <Col span={16}>
              <Card title="Thông tin khám bệnh" className="consultation-details-card">
                <Form.Item
                  label="Triệu chứng"
                  name="symptoms"
                  rules={[{ required: true, message: 'Vui lòng nhập triệu chứng' }]}
                >
                  <TextArea rows={3} placeholder="Mô tả triệu chứng của bệnh nhân..." />
                </Form.Item>
                ...
                >
                  <TextArea rows={3} placeholder="Phương pháp điều trị được chỉ định..." />
                </Form.Item>
                ...
                <Form.Item
                  ...
                  name="prescription"
                >
                  <TextArea rows={3} placeholder="Danh sách thuốc và cách dùng..." />
                </Form.Item>

                <Form.Item
                  label="Ghi chú khám bệnh"
                  name="notes"
                >
              ...
              >
                {existingTreatmentPlan ? (
                  ...
                  <>
                    ...
                    <Form.Item label="Ngày giai đoạn tiếp theo" name="nextPhaseDate">
                      <DatePicker style={{ width: '100%' }} />
                    </Form.Item>

                    <Form.Item label="Ghi chú tiến trình" name="progressNotes">
                      <TextArea rows={2} placeholder="Ghi chú về tiến trình..." />
                    </Form.Item>
                  ...
                  // Create new plan
                  <>
                    <Form.Item label="Dịch vụ điều trị" name="treatmentServiceId" rules={[{ required: true, message: 'Vui lòng chọn dịch vụ điều trị' }]}>
                      <Select placeholder="Chọn dịch vụ" showSearch optionFilterProp="children">
                        {treatmentServices.length === 0 ? (
                          <Option disabled value="">Đang tải dịch vụ...</Option>
                        ) : (
                          treatmentServices.map(service => (
                            <Option key={service.id} value={service.id}>
                              {service.serviceName || service.name || `Dịch vụ ${service.id}`}
                              {service.estimatedCost && ` - ${service.estimatedCost.toLocaleString()} VND`}
                            </Option>
                          ))
                        )}
                      </Select>
                    ...
                    <Form.Item label="Mô tả kế hoạch" name="treatmentDescription">
                      <TextArea rows={3} placeholder="Mô tả chi tiết kế hoạch điều trị..." />
                    </Form.Item>
          ...
          <Card title="Tái khám" size="small" style={{ marginTop: 16 }}>
            <Row gutter={16}>
              ...
              <Col span={16}>
                <Form.Item shouldUpdate={(prevValues, currentValues) => prevValues.followUpRequired !== currentValues.followUpRequired}>
                  {({ getFieldValue }) => (
                    <Form.Item 
                      label="Ngày tái khám" 
                      ...
                      rules={[
                        {
                          validator(_, value) {
                            if (getFieldValue('followUpRequired') && !value) {
                              return Promise.reject(new Error('Vui lòng chọn ngày tái khám'));
                            }
